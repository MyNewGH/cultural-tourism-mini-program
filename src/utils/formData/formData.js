...
/**
 * @desc base64 图片转 Array Buffer
 */
function base64ImageToArrayBuffer(base64Image) {
  // 移除数据URI头部
  const base64WithoutHeader = base64Image.replace(/^data:image\/(png|jpg|jpeg);base64,/, '');

  // 将Base64字符串解码为二进制数据
  const binaryString = atob(base64WithoutHeader);

  // 创建一个Uint8Array以存放二进制数据
